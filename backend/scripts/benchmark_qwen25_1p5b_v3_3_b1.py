#!/usr/bin/env python3
"""
V3.3-B.1 — Qwen2.5-1.5B Semantic Validation Benchmark
================================================================
...
- The benchmark validates the semantic output contract and semantic gate.
"""
...
BENCHMARK_VERSION = "V3.3-B.1"
PROMPT_VERSION = "candidate-semantic-validation-v2"
...
DEFAULT_OUTPUT = Path(
    "/opt/investment-os/generated/restomart/"
    "benchmark_qwen25_1p5b_v3_3_b1.json"
)
...
- Do not write alternatives.
- Do not repeat the question.
- Do not repeat the candidate.
- Do not use markdown.
- The first character must be {{.
- The last character must be }}.
- The object must contain exactly two fields: accepted and reason.
...
def parse_semantic_json(
    text: str,
) -> tuple[bool, bool | None, str, str]:
    """
    Strict semantic-output parser.

    Returns:
        (json_valid, accepted, reason, invalid_reason)
    """
    ...
    if not cleaned.startswith("{"):
        return False, None, "", "not_json_start"

    if not cleaned.endswith("}"):
        return False, None, "", "not_json_end"
    ...
    if set(obj.keys()) != {"accepted", "reason"}:
        return False, None, "", "wrong_keys"

    accepted = obj.get("accepted")
    reason = obj.get("reason")

    if not isinstance(accepted, bool):
        return False, None, "", "invalid_accepted_type"
...
def validate_input_header(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if errors:
        raise RuntimeError(
            "V3.3-B.1 smoke input validation failed:\n  - "
            + "\n  - ".join(errors)
        )
...
def validate_candidate(
    ...
    valid, accepted, reason, invalid_reason = parse_semantic_json(
        raw_response
    )
...
def summarize(
    ...
    invalid_reason_counts: Counter[str] = Counter(
        ...
        for r in records
        if r["semantic"]["status"] == "json_invalid"
    ...
    return {
        "population": {
            ...
            "end_to_end_evidence_yield_pct": pct(len(unique_evidence), EXPECTED_RAW_CANDIDATES),
        },
        "timing": {
            ...
            if latencies else 0.0,
            "projected_total_min": round(sum(latencies) / 60.0, 3),
        },
...
def print_header(
    ...
    print("=" * 78)
    print("V3.3-B.1 SMOKE — QWEN SEMANTIC OUTPUT CONTRACT")
    print("=" * 78)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        input_validation, candidates = load_gated_candidates(
            input_path
        )
    except Exception as exc:
    ...
    result = {
        "benchmark": {
            ...
            "purpose": "full_251_candidate_semantic_validation",
        },
        "startup": {
            "name": STARTUP_NAME,
    ...
    print(f"INFERENCE MIN      : {summary['timing']['projected_total_min']:.3f}")
    print(f"OUTPUT             : {output_path}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "V3.3-B.1 251-candidate Qwen2.5-1.5B semantic validation benchmark"
        )
    )
